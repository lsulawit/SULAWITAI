...
function detectLanguage(text, target = 'en') {
  ...
  const scores = {
    vi: /[ăâđêôơưàáảãạèéẻẽẹìíỉĩịòóỏõọùúủũụỳýỷỹỵ]/i.test(text) ? 8 : 0,
    tr: /[ğışçöü]/i.test(text) ? 5 : 0,
    es: /[¿¡ñ]/i.test(text) ? 5 : 0,
    fr: /[àâçéèêëîïôûùüÿœ]/i.test(text) ? 4 : 0,
    de: /[äöüß]/i.test(text) ? 4 : 0,
    pt: /[ãõçáâàéêíóôú]/i.test(text) ? 3 : 0,
  ...
  const words = {
    ...
    de: [' der ',' die ',' das ',' und ',' ist ',' nicht ',' danke ',' hallo ',' ich ',' mit ',' für ',' ein '],
    pt: [' o ',' a ',' os ',' as ',' uma ',' que ',' para ',' obrigado ',' olá ',' estou ',' você ',' com '],
    ...
    tr: [' bir ',' ve ',' değil ',' ben ',' sen ',' için ',' ile ',' teşekkür ',' merhaba '],
    vi: [' tôi ',' bạn ',' không ',' và ',' của ',' cho ',' cảm ơn ',' xin chào '],
...
async function translateWithMyMemory(text, sourceCode, targetCode) {
  ...
  if (chunks.length > 12) throw new Error('ข้อความยาวเกินไปสำหรับโหมดฟรี กรุณาแบ่งข้อความให้สั้นลง');
  ...
  for (const chunk of chunks) {
    ...
    if (!r.ok) throw new Error(data?.responseDetails || 'บริการแปลฟรีไม่พร้อมใช้งาน');
    ...
    if (!output || /quota|limit|available free translation/i.test(details + ' ' + output)) {
      throw new Error('โควตาแปลฟรีวันนี้หมดแล้ว กรุณาลองใหม่ภายหลัง');
    }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!text || typeof text !== 'string') return res.status(400).json({ error: 'กรุณาใส่ข้อความที่ต้องการแปล' });
    if (text.length > 4000) return res.status(400).json({ error: 'ข้อความยาวเกิน 4,000 ตัวอักษร' });
    ...
    console.error(error);
    return res.status(502).json({ error: error?.message || 'ไม่สามารถแปลได้ในขณะนี้' });
  }
}
