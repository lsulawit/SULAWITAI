...
const buddyPhrases = [
  'เย่! เดี๋ยวหนูช่วยแปลให้นะ ✨',
  'พร้อมแล้ว มาลองแปลคำใหม่กันเถอะ 🌈',
  'พิมพ์ได้เลย หนูรอฟังอยู่ 🐰',
  'เก่งมาก ลองอีกคำได้เลยนะ 💖'
];
...
function shortText(text='', max=48){ return text.length > max ? `${text.slice(0, max)}…` : text; }
function randomBuddy(){ return buddyPhrases[Math.floor(Math.random() * buddyPhrases.length)]; }
...
function renderLangButtons(){
  ...
  $('#sourceFlag').textContent = s.flag;
  $('#sourceLabel').textContent = s.name;
  $('#targetFlag').textContent = t.flag;
  $('#targetLabel').textContent = t.name;
}

function updateBuddy(title, message, sourceText = '', targetText = ''){
  $('#buddyTitle').textContent = title;
  $('#buddyMessage').textContent = message;
  if (sourceText) $('#wordSource').textContent = shortText(sourceText, 58);
  if (targetText) $('#wordTarget').textContent = shortText(targetText, 58);
}
...
function renderLanguageList(filter=''){
  ...
  list.innerHTML = options.map(l => `
    <button class="language-option" data-code="${l.code}">
      <span>${l.flag}</span>
      <span><strong>${escapeHTML(l.name)}</strong><small>${escapeHTML(l.en)}</small></span>
    </button>`).join('');
  $$('.language-option').forEach(btn => btn.onclick = () => {
    ...
    if (state.source === state.target && state.source !== 'auto') {
      if (state.selecting === 'source') state.target = 'en';
      else state.source = 'auto';
    }
    renderLangButtons();
    closeLanguage();
    updateBuddy('เลือกภาษาแล้ว ✨', `ตอนนี้หนูจะช่วยแปลจาก ${lang(state.source).name} เป็น ${lang(state.target).name}`);
  });
}
...
$('#swapBtn').onclick = () => {
  ...
  renderLangButtons();
  updateBuddy('สลับภาษาแล้ว 🔄', `ตอนนี้จะแปลจาก ${lang(state.source).name} เป็น ${lang(state.target).name}`);
  $('#swapBtn').animate([{transform:'rotate(0deg) scale(1)'},{transform:'rotate(180deg) scale(1.08)'},{transform:'rotate(180deg) scale(1)'}],{duration:420,easing:'ease'});
};

function addMessage(role, text, meta = {}){
  const isAI = role === 'assistant';
  const article = document.createElement('article');
  article.className = `message ${isAI ? 'ai-message' : 'user-message'}`;
  article.innerHTML = `
...
async function translate(){
  ...
  updateBuddy('กำลังแปลให้แล้ว… ✨', 'รอสักครู่นะ หนูกำลังช่วยแปลอยู่', text, 'กำลังประมวลผล...');
  input.value = '';
  autosize();
  ...
  try {
    ...
    const res = await fetch('/api/translate', {
      ...
      body: JSON.stringify({
        text,
        sourceLanguage: lang(state.source).en,
        targetLanguage: lang(state.target).en,
        tone: state.tone,
        context
      })
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || 'Translation service unavailable');

    typing.remove();
    const output = data.translation || 'ไม่พบคำแปล';
    addMessage('assistant', output, {source:state.source, target:state.target, engine:data.engine});
    updateBuddy('แปลเสร็จแล้ว 🎉', `${randomBuddy()} ตอนนี้คำแปลพร้อมแล้วนะ`, text, output);
  } catch (err) {
    typing.remove();
    addMessage('assistant', `แปลไม่สำเร็จ: ${err.message || 'กรุณาลองใหม่อีกครั้ง'}`, {error:true});
    updateBuddy('โอ๊ะ! มีปัญหานิดหน่อย', 'ลองพิมพ์ใหม่อีกครั้ง หรือเปลี่ยนภาษาดูนะ');
    showToast('แปลไม่สำเร็จ กรุณาลองอีกครั้ง');
  } finally {
...
function speakText(text){
  if (!('speechSynthesis' in window)) return showToast('อุปกรณ์นี้ไม่รองรับเสียงพูด');
  speechSynthesis.cancel();
  const u = new SpeechSynthesisUtterance(text);
  u.lang = lang(state.target).code;
...
function setupSpeech(){
  ...
  const mic = $('#micBtn');
  if (!SR) {
    mic.onclick = () => showToast('เบราว์เซอร์นี้ยังไม่รองรับการพูดเป็นข้อความ');
    return;
  }
  const rec = new SR();
  ...
  mic.onclick = () => {
    rec.lang = state.source === 'auto' ? 'th-TH' : state.source;
    rec.start();
    mic.classList.add('listening');
    updateBuddy('กำลังฟังอยู่ 🎤', 'พูดได้เลย หนูกำลังฟังและจะพิมพ์ให้');
  };
  rec.onresult = e => {
  ...
  rec.onerror = () => {
    ...
    showToast('ไม่สามารถใช้ไมโครโฟนได้');
  };
}
setupSpeech();
...
$('#historyNav').onclick = openDrawer;
$('#closeDrawer').onclick = closeDrawer;
backdrop.onclick = closeDrawer;

function getSessions(){
  try { return JSON.parse(localStorage.getItem('sulawit-history') || '[]'); }
  catch { return []; }
}
function saveHistory(){
  ...
  const item = {
    id: state.sessionId,
    title: state.messages.find(m => m.role === 'user')?.text.slice(0, 55) || 'แชตใหม่',
    updated: Date.now(),
...
function renderHistory(){
  ...
  if (!sessions.length) {
    historyList.innerHTML = '<div class="empty-history">ยังไม่มีประวัติการแปล<br>เริ่มแชตแรกของคุณได้เลย ✨</div>';
    return;
  }
  historyList.innerHTML = sessions.map(s => `
    <div class="history-item" data-id="${s.id}">
      <strong>${escapeHTML(s.title)}</strong>
      <span>${new Intl.DateTimeFormat('th-TH',{dateStyle:'medium',timeStyle:'short'}).format(new Date(s.updated))}</span>
    </div>`).join('');
...
function resetChat(){
  ...
  state.messages = [];
  chatArea.innerHTML = `
    <article class="message ai-message intro-message">
      <div class="avatar ai-avatar"><span>🐰</span></div>
      <div class="bubble">
        <div class="bubble-head"><strong>SULAWIT AI</strong><span>ตอนนี้</span></div>
        <p>เริ่มบทสนทนาใหม่แล้ว พิมพ์ข้อความที่ต้องการแปลได้เลย ✨</p>
      </div>
    </article>`;
  updateBuddy('เริ่มใหม่แล้ว ✨', 'พร้อมช่วยแปลอีกครั้ง ลองพิมพ์คำใหม่ได้เลย', 'ลองพิมพ์คำว่า ขอบคุณ', 'แล้วดูคำแปลตรงนี้ได้เลย');
  closeDrawer();
  input.focus();
}
function loadSession(id){
  ...
  state.messages = [];
  state.source = s.source || 'auto';
  state.target = s.target || 'en';
  state.tone = s.tone || 'natural';
  renderLangButtons();
  chatArea.innerHTML = '';
  (s.messages || []).forEach(m => addMessage(m.role, m.text, m.meta || {}));
  const lastUser = [...(s.messages || [])].reverse().find(m => m.role === 'user')?.text || 'แชตที่บันทึกไว้';
  const lastAI = [...(s.messages || [])].reverse().find(m => m.role === 'assistant')?.text || 'กลับมาดูคำแปลเดิมได้เลย';
  updateBuddy('เปิดประวัติแล้ว 📚', 'นี่คือคำแปลครั้งก่อนของคุณ', lastUser, lastAI);
  closeDrawer();
}
$('#newChatBtn').onclick = resetChat;
$('#clearHistoryBtn').onclick = () => {
  localStorage.removeItem('sulawit-history');
  renderHistory();
...
$('#focusBtn').onclick = () => {
  ...
  showToast(document.body.classList.contains('focus-mode') ? 'โหมดโฟกัส' : 'โหมดปกติ');
};
...
renderHistory();
autosize();
updateBuddy('หนูชื่อ LULU ✨', 'สวัสดี! พิมพ์คำหรือประโยคที่อยากแปลได้เลย เดี๋ยวหนูช่วยแปลให้นะ');
